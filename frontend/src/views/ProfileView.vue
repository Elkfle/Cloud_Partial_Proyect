...
export default {
    ...
    computed: {
        ...
        // entregar desde el contenedor ususario
        imageUrl() { return this.user_data.image ? `lb-prod-proy-167809630.us-east-1.elb.amazonaws.com:8000/api/${this.user_data.image}` : this.defaultImage; },
    },
    methods: {
        async mounted() {
            const token = localStorage.getItem('token');
            const response = await userAPI.get('/current_user', {
                headers: {
        ...
        async updatePersonalInfo() {
            ...
            try {
                ...
                const response_updated = await userAPI.get(`/users/${userId}`);
        ...
        async updateSecurity() {
            ...
            try {
                ...
                const response = await userAPI.post(`/users/${userId}/change_password`, { password: this.password, new_password: this.new_password });
                this.success = response.data.message;
