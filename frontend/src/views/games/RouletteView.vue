            ...
            <div class="relative">
                ...
                <div class="mt-10 flex space-x-2">
                    <img v-for="token in tokens" :key="token"
                        :src="`lb-prod-proy-167809630.us-east-1.elb.amazonaws.com:8800/api/static/games/rouletteGame/token${token}.png`"
...
.background_roulette {
    ...
    background-image: url('lb-prod-proy-167809630.us-east-1.elb.amazonaws.com:8800/api/static/games/rouletteGame/casino-roulette-felt.jpg');
...
export default {
    ...
    methods: {
        ...
        async submitBet() {
            ...
            this.betting = true
            const response = await gamesAPI.post('/roulette/bet', { bet_data: this.selectedNumbers })
            try {
                const betId = response.data.id;
                const resultResponse = await gamesAPI.get(`roulette/result?bet_id=${betId}`);
